...
With this release, Receive Packet Steering (RPS) is now disabled by default. The default configuration affects containers that perform networking system calls, such as send, directly within latency-sensitive threads. To avoid latency impacts, move networking calls to helper threads or processes.
...
To revert to the previous behavior, add the `performance.openshift.io/enable-rps` annotation to the PerformanceProfile manifest. This action restores the prior functionality at the cost of globally reducing networking performance for all pods.

To revert the setting, modify the PerformanceProfile as follows:
