...
. Verify routes are being advertised by checking the speaker logs:
+
[source,terminal]
----
$ oc logs -n metallb-system -l component=speaker | grep "service has IP, announcing"
----
+
.Example output
+
[source,terminal]
----
{"caller":"main.go:444","event":"serviceAnnounced","ips":["192.168.122.210"],"level":"info","msg":"service has IP, announcing","pool":"bgp-pool","protocol":"bgp"}
----
