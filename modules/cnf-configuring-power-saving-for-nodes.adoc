...
[role="_abstract"]
Enable power savings for nodes running colocated high and low priority workloads without impacting latency or throughput by configuring per-pod power management settings.
...
+
The following shows an example `PerformanceProfile` configuration using the `perPodPowerManagement` setting.
+
[source,yaml]
----
