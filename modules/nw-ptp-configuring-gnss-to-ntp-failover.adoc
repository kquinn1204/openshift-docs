...
[NOTE]
====
The output shows one NodePtpDevice resource for each node with PTP-capable interfaces. In this example, 5 worker nodes have Intel E810 NICs. For {sno} clusters, you would see only one NodePtpDevice resource.
====
...
. Check the NodePtpDevice to see if the profile is applied by running the following command, replacing `<node_name>` with your node's hostname:
+
[source,terminal]
----
