...
[role="_abstract"]
Configure workload hints in performance profiles to balance power consumption and latency requirements by setting realTime, highPowerConsumption, and perPodPowerManagement flags based on your environment needs.
...
Setting `perPodPowerManagement` to `false` disables some debugging and monitoring features that can affect system latency.
