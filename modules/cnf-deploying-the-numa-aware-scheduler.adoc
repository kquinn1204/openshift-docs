...
[NOTE]
====
For disconnected environments make sure that you have mirrored the numa scheduler container image `registry.redhat.io/openshift4/noderesourcetopology-scheduler-rhel9:v{product-version}` into a disconnected registry. 

This image is not mirrored as part of NUMA Resources Operator at the moment due to soft dependency limitations for Operator container images. When using the `oc-mirror` the image can be specified in `additionalImages:` section
====
